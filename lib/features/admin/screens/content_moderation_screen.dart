...
class _ContentModerationScreenState
  ...
  static const _tabFilters = ['all', 'flagged', 'hidden', 'anonymous', 'pinned'];
  static const _tabLabels = ['All Posts', 'Flagged', 'Hidden', 'Anonymous', 'Pinned'];
  ...
  List<CommunityPostAdmin> _filteredPosts(List<CommunityPostAdmin> raw) {
    ...
    if (_searchQuery.isNotEmpty) {
      posts = posts
          .where((p) => p.content.toLowerCase().contains(_searchQuery))
          .toList();
    }
    switch (_sort) {
  ...
  Future<void> _bulkDelete(List<CommunityPostAdmin> posts) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text(
          'This will permanently delete ${_selected.length} post(s). Cannot be undone.',
          style:
                ...
                children: [
                  ...
                  IconButton(
                    ...
                    icon: Icon(
                      ...
                      size: 18,
                      color:
                          _bulkMode ? AppColors.error : AppColors.textMuted,
                    ),
              ...
              if (_bulkMode && _selected.isNotEmpty)
                _BulkActionsBar(
                  ...
                  onSelectAll: () => setState(
                    () => _selected.addAll(posts.map((p) => p.id)),
                  ),
...
class _StatChipBtn extends StatelessWidget {
  final String label;
  final int value;
  final Color color;
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Row(
          children: [
            ...
            const SizedBox(width: 6),
            _BulkBtn(
                'Hide', LucideIcons.eyeOff, AppColors.textMuted, onHide),
            const SizedBox(width: 6),
            _BulkBtn(
                'Delete', LucideIcons.trash2, AppColors.error, onDelete),
          ],
        ),
      );
}
...
class _PostCardState extends ConsumerState<_PostCard> {
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTap: widget.bulkMode ? widget.onToggleSelect : null,
      child: Container(
        decoration: BoxDecoration(
          color: bgColor,
                ...
                children: [
                    ...
                    children: [
                      ...
                      Wrap(
                        ...
                        children: [
                          if (post.isFlagged)
                            _Badge(
                                'FLAGGED', AppColors.error, LucideIcons.flag),
                          if (post.isHidden)
                            _Badge('HIDDEN', AppColors.textMuted,
                                LucideIcons.eyeOff),
                          if (post.isPinned)
                            _Badge('PINNED', AppColors.tertiary,
                                LucideIcons.pin),
                          if (post.isAnonymous)
                  ...
                  Row(
                    children: [
                      ...
                      const SizedBox(width: 10),
                      _InlineStat(
                          LucideIcons.messageCircle, post.commentCount),
                      const Spacer(),
                    ],
                  ),
                  ...
                  SingleChildScrollView(
                    ...
                    child: Row(
                      children: [
                        ...
                        if (post.isFlagged) const SizedBox(width: 6),
                        _ActionBtn(
                          icon: post.isHidden
                              ? LucideIcons.eye
                              : LucideIcons.eyeOff,
  ...
  Future<void> _confirmDelete(BuildContext ctx) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text(
          'This action is permanent and cannot be undone. The post and all its comments/reactions will be removed.',
  ...
  Widget build(BuildContext context) {
    if (post.isAnonymous) {
      return Container(
        ...
        child: const Center(
          child: Icon(LucideIcons.userX, size: 16, color: AppColors.info),
        ),
      );
    }
...
class _PostDetailSheet extends ConsumerWidget {
                  ...
                  children: [
                    ...
                    Wrap(
                      ...
                      children: [
                        ...
                        _Badge(post.postType.toUpperCase(), AppColors.info,
                            null),
                      ],
                    ),
                    ...
                    Row(
                      children: [
                        ...
                        const SizedBox(width: 20),
                        _DetailMetaItem(
                            LucideIcons.tag, 'Type', post.postType),
                      ],
                    ),
  ...
  void _showWarnDialog(BuildContext ctx, WidgetRef ref) {
    final msgCtrl = TextEditingController(
      text:
          'Your post has been reviewed by our moderation team and found to violate our community guidelines. Please ensure your future posts adhere to our standards.',
    );
    showDialog(
      context: ctx,
      builder: (_) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text(
              'Sending to: ${post.authorName ?? 'Unknown User'}',
              style: const TextStyle(
                  fontSize: 12, color: AppColors.textMuted),
            ),
...
class _EmptyState extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final subtitle = filter == 'flagged'
        ? 'Community content looks clean'
