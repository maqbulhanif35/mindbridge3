...
class EmailService {
  static String get _apiKey => dotenv.maybeGet('RESEND_API') ?? '';
  static const _appUrl = 'https://mindbridge-teal.vercel.app';

  static const _from = 'MindBridge <noreply@smartload.space>';
  ...
  static Future<bool> _send({
    ...
    if (_apiKey.isEmpty) {
      print('[EmailService] RESEND_API not set — skipping email to $to');
      return false;
    }
    try {
      ...
      if (response.statusCode == 200 && response.body.isNotEmpty) {
        ...
        if (result['id'] != null) {
          print('[EmailService] ✓ Sent to $to — id: ${result['id']}');
          return true;
        }
      }
      print('[EmailService] ✗ Failed to $to: ${response.statusCode} ${response.body}');
      return false;
    } catch (e) {
      print('[EmailService] ✗ Request error: $e');
      return false;
    }
  }
  ...
  // ── OTP Email (password reset / email verification) ──────
